/*
  ...
  Edited by Yudetetamago-AM for Raspberry Pi Pico C/C++ SDK
...
#include "pico-bno08x.h"

//Attempt communication with the device
//Return true if we got a 'Polo' back from Marco
bool BNO080::begin(uint8_t deviceAddress, i2c_inst_t* i2cBus, uint8_t intPin)
{
...
bool BNO080::beginSPI(uint8_t user_CSPin, uint8_t user_WAKPin, uint8_t user_INTPin, uint8_t user_RSTPin, uint32_t spiPortSpeed, SPIClass &spiPort)
{
	...
	gpio_set_dir(_cs, OUTPUT);
	gpio_set_dir(_wake, OUTPUT);
	gpio_set_dir(_int, INPUT_PULLUP);
	gpio_set_dir(_rst, OUTPUT);

	gpio_put(_cs, 1); //Deselect BNO080

	//Configure the BNO080 for SPI communication
	gpio_put(_wake, 1); //Before boot up the PS0/WAK pin must be high to enter SPI mode
	gpio_put(_rst, LOW);   //Reset BNO080
	sleep_ms(2);				   //Min length not specified in datasheet?
	gpio_put(_rst, 1);  //Bring out of reset

	//Wait for first assertion of INT before using WAK pin. Can take ~104ms
	waitForSPI();

	//if(wakeBNO080() == false) //Bring IC out of sleep after reset
	//  printf("BNO080 did not wake up\n");
	...
	if (receivePacket() == true)
	{
		if (shtpData[0] == SHTP_REPORT_PRODUCT_ID_RESPONSE)
			if (_printDebug == true)
			{
				...
				printf(SW_Part_Number);
				uint32_t SW_Build_Number = (uint32_t)shtpData[11] << 24) | (uint32_t)shtpData[10] << 16) | (uint32_t)shtpData[9] << 8) | (uint32_t)shtpData[8]);
				printf(" SW Build Number: 0x");
				printf(SW_Build_Number);
				uint16_t SW_Version_Patch = (uint16_t)shtpData[13] << 8) | (uint16_t)shtpData[12]);
				printf(" SW Version Patch: 0x");
				printfln(SW_Version_Patch);
			}
			return (true);
	}
...
// Calling this function enables printf debug
void BNO080::enableDebugging(void)
{
...
void BNO080::parseCommandReport(void)
{
	...
	{	
		printf("parseCommandReport: GET_FEATURE_RESPONSE!\n");
		featureReportId = shtpData[1];
		reportInterval = (uint32_t)shtpData[8] << 24 | (uint32_t)shtpData[7] << 16 | (uint32_t)shtpData[6] << 8 | shtpData[5];
	}
	//Add by Math: end
	else
	{
...
void BNO080::getFeatureRequest(uint8_t reportID) {
	shtpData[0] = SHTP_REPORT_GET_FEATURE_REQUEST;
	shtpData[1] = reportID;
...
bool BNO080::waitForWriteI2C() {
	for (uint8_t counter = 0; counter < 100; counter++)
	{
		if (i2c_get_write_available(_i2cBus) > 0)
			return (true);
		sleep_ms(1);
	}
...
#endif
	{
		int16_t ret = i2c_read_blocking(_i2cBus, (uint8_t)_deviceAddress, shtpHeader, 4, false);
...
#if 1
		//waitForReadI2C();
		ret = i2c_read_blocking(_i2cBus, (uint8_t)_deviceAddress, shtpBuffer, dataLength, false);
		if (ret < 0) {
			//printf("receivePacket: ret: %d\n", ret);
			return (false);
		}
		if (_printDebug == true)
			printf("receive: ");
...
bool BNO080::getData(uint16_t bytesRemaining)
{
	...
	static uint8_t buf[I2C_BUFFER_LENGTH];
	...
	while (bytesRemaining > 0)
	{
		...
		if (waitForReadI2C() == false) 
			return (false); //Error

		i2c_read_blocking(_i2cBus, _deviceAddress, buf, (uint8_t)(numberOfBytesToRead + 4), false);

		for (uint8_t x = 0; x < numberOfBytesToRead; x++)
		{
			if (dataSpot < MAX_PACKET_SIZE)
			{
				shtpData[dataSpot++] = buf[x+4]; //Store data into the shtpData array
			}
			else
			{
...
bool BNO080::printMECalibrationRespond()
{
	...
	while(1)
	{
        ...
        if(dataAvailable() == true)
        {
			if (shtpData[0] == SHTP_REPORT_COMMAND_RESPONSE)
			{
				...
				{
					printf("Configure ME Calibration Command Respond Report :\n");
					printf("-Status : %d\n", calibrationStatus);
					printf("-Accel Cal Enable : %d\n", calibrationAccEnable);
					printf("-Gyro Cal Enable : %d\n", calibrationGyroEnable);
					printf("-Magn Cal Enable : %d\n", calibrationMagnEnable);
					printf("-Plan Cal Enable : %d\n", calibrationPlanEnable);
					return (true);
				}
			}
		}
...
bool BNO080::getFeatureResponseAvailable()
{
...
//getFeatureResponseAvailable() function shall be execute before to be sure to have received the last Get Feature Response report
uint32_t BNO080::getReportInterval()
{
...
bool BNO080::checkReportEnable(uint8_t reportID, uint16_t timeBetweenReports)
{
	uint32_t reportIntervalMicrosecond = timeBetweenReports * 1000L;
...
{
	uint32_t reportIntervalMillisecond = (getReportInterval()/1000L);
	printf("Report ID : 0x%x\n", getFeatureReportId());
	printf("Report interval : %d ms\n", reportIntervalMillisecond);
}
