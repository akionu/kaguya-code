...
{
public:
	bool begin(uint8_t deviceAddress = BNO080_DEFAULT_ADDRESS, i2c_inst_t* i2cBus = i2c0, uint8_t intPin = 255); //By default use the default I2C addres, and use Wire port, and don't declare an INT pin
	...
	uint8_t resetReason(void); //Query the IMU for the reason it last reset
	...
	bool waitForReadI2C(); //Delay based polling for I2C traffic
	bool waitForWriteI2C();
	...
	bool printMECalibrationRespond(); //Add by Math : function to print the Configure ME Calibration Command Respond Report
	...
	void getFeatureRequest(uint8_t reportID);
	bool getFeatureResponseAvailable(); //Add by Math: a function to check if a Get Feature Response report is received from BNO08x
	...
	uint32_t getReportInterval(); //Add by Math: a function to return the Report Interval of the last Get Feature Response report
	bool checkReportEnable(uint8_t reportID, uint16_t timeBetweenReports); //Add by Math: a function to check if the Get Feature Response report received from BNO08x correspond to what we have enable
	...
	//Global Variables
	uint8_t shtpBuffer[MAX_PACKET_SIZE]; //Each packet has a header of 4 bytes
	uint8_t* shtpHeader = shtpBuffer;
	uint8_t* shtpData = &shtpBuffer[4];
...
private:
	//Variables
	i2c_inst_t* _i2cBus;		//The generic connection to user's chosen I2C hardware
	...
	uint32_t reportInterval;                                  //Byte [5:8] of Get Feature Response
	//Add by Math: end
